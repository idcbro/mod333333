...
exports.run = async(client, message, args, author, victim) => {
    if(!author.roles.cache.has(ayarlar.guildHammer.muteHammer) && !author.hasPermission("MANAGE_ROLES")) return permEmbed(message)
let time = args[1]
let reason = args.splice(2).join(" ");

    let logKanal = client.channels.cache.get(ayarlar.guildLogs.muteLogs)
...
let data = await Database.find({guildID: ayarlar.guildSettings.guildID, victimID: victim.id, activity: true, Type: "MUTE"})
if(data.length >= 1 ) return cezaEmbed(message)
await ceza.findOneAndUpdate({ guildID: message.guild.id, execID: author.id }, { $inc: { Ban: 0, Jail: 0, Mute: 1, Vmute: 0 } }, { upsert: true });
await kayit.findOneAndUpdate({ guildID: message.guild.id, execID: victim.id }, { $inc: { Jail: 0, Ban: 0, Mute: 1, Vmute: 0 } }, { upsert: true });

let count = await Database.countDocuments().exec();
count = count == 0 ? 1 : count + 1;
rolVer(victim.id,ayarlar.guildRoles.muteRole)
...
embedCreator("#4D0A0A", ` ${victim} kişisi **${yaziSure}** boyunca **metin kanallarında** susuturuldu.\nSusturulma sebebi: \`${reason}\` (Ceza numarası: \`#${count}\`) `,message,[true,20000])
message.react(ayarlar.emojiler.onay)
logEmbed(logKanal, ` 

${victim} Kişisi yazı kanallarında ${yaziSure} boyunca susturuldu! 
...
exports.commandSettings = {
    ...
    guildOnly: true, 
    coolDown: 3000, 
    description: "mute @Λrda/852956127728762890 [Süre] [Sebep]"
}
