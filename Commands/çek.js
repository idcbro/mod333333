...
exports.run = async(client, message, args, author, victim) => {
    ...
    if (author.hasPermission('ADMINISTRATOR') || author.roles.cache.has(ayarlar.guildHammer.auth)) return author.voice.setChannel(victim.voice.setChannel(author.voice.channelID)).catch(err => {}) && message.channel.send(new MessageEmbed().setDescription(`Başarıyla <@${victim.id}> adlı üye kanalınıza taşındı.`).setAuthor(message.author.tag, author.user.avatarURL({dynamic: true})).setColor(`RANDOM`)).then(msg => msg.delete({timeout: 5000})) && message.react(ayarlar.emojiler.onay)
...
exports.commandSettings = {
    ...
    guildOnly: true, 
    coolDown: 3000, 
    description: "çek @Λrda/852956127728762890"
}