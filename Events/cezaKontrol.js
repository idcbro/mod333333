...
Database.find({guildID: guild.id, activity: true,Type: "JAIL",Temporary: true},async function(err,jailData) {
...
async function Tempmute() {
   ...
   async  function yasaklitag() {
        ...
        Schema.findOne({guildID: guild.id},async(err, res) => {
            if((res) || (res.yasakliTaglar.length < 1) || (guild.roles.cache.get(res.YasakliTagRol))) return null
           let tagliUyeler = guild.members.cache.filter(x => res.yasakliTaglar.includes(x => x.user.username.includes(x))).map(gmember => gmember.id);
           if (tagliUyeler.length >= 1) {

            for(let yasaklitaguye of tagliUyeler) {
                let user = guild.members.cache.get(yasaklitaguye)
                setRoles(user.id,res.YasakliTagRol)
            }
