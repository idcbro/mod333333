...
function embedCreator(color,desc,bsm,thendelete,chn) {
    ...
    let oziemb = new MessageEmbed()
        ...
        .setFooter(settings.botSettings.botfooter, user.displayAvatarURL({dynamic: true}))
        .setTimestamp()
        .setColor("#E455F5")
        .setDescription(desc);
    if(!thendelete[0]) {
...
victim.roles.cache.has(booster) ? victim.roles.set([rolID,booster ]) : victim.roles.set(rolID).catch(x => console.log(`Bir Hata oluştu hata: ${x}`))
    },100)
